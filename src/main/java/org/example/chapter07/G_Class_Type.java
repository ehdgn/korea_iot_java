...
public class G_Class_Type {
    byte a = 1;
    short b = a;
    char c = (char) a; // 타입의 체계가 달라도(숫자 - 문자의 관계라도) 변환 가능
    public static void main(String[] args) {
        ...
        // : 부모 클래스 타입으로 변환된 객체를 다시 자식클래스 타입으로 변환하는 것
        // - 자식 클래스에만 정의된 멤버에 다시 접근 가능

        // 자식 타입 객체변수명 = (자식타입) 부모객체;
        G_Child gChild = (G_Child) parentObject;
        gChild.displayInfo(); // 자식 클래스입니다. (재정의)
